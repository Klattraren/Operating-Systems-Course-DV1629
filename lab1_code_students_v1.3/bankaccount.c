...
void deposit(double amount) {
    pthread_mutex_lock(&lock); //Added mutex lock in oreder to make sure that only one thread can access the bankAccountBalance at a time
    bankAccountBalance += amount;
    pthread_mutex_unlock(&lock); //Unlocking the shared variable so that another thread can access it
}

void withdraw(double amount) {
    pthread_mutex_lock(&lock); //Did the same mutexlock as in deposit here
    bankAccountBalance -= amount;
    pthread_mutex_unlock(&lock); //And the same unlock
}
